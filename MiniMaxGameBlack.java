...
public class MiniMaxGameBlack {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Board bestBoard = minimaxMidgameEndgame(board, depth, 'W');
        ...
        System.out.println("MINIMAX estimate: " + bestBoard.staticEvaluationMidgame());
    }

    private static Board minimaxMidgameEndgame(Board board, int depth, char player) {
        estimate++;
        if (depth == 0) {
            return board;
        }
        ...
        if (player == 'W') {
            ...
            for (Board move : moves) {
                Board currBoard = minimaxMidgameEndgame(move, depth - 1, 'B');
                int currValue = currBoard.staticEvaluationMidgame();
            ...
            for (Board move : moves) {
                Board currBoard = minimaxMidgameEndgame(move, depth - 1, 'W');
                int currValue = currBoard.staticEvaluationMidgame();
                if (currValue < bestValue) {
                    bestValue = currValue;
                    bestBoard = move;
