...
public class ABOpeningBlack {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Board bestBoard = alphaBetaOpening(board, depth, 'W', Integer.MIN_VALUE, Integer.MAX_VALUE);
        ...
        System.out.println("MINIMAX estimate: " + bestBoard.staticEvaluationOpeningImproved());
    }

    private static Board alphaBetaOpening(Board board, int depth, char player, int alpha, int beta) {
        estimate++;
        ...
        if (player == 'W') {
            ...
            for (Board move : moves) {
                Board currBoard = alphaBetaOpening(move, depth - 1, 'B', alpha, beta);
                int currValue = currBoard.staticEvaluationOpeningImproved();
            ...
            for (Board move : moves) {
                Board currBoard = alphaBetaOpening(move, depth - 1, 'W', alpha, beta);
                int currValue = currBoard.staticEvaluationOpeningImproved();
                if (currValue < bestValue) {
                    bestValue = currValue;
                    bestBoard = move;
